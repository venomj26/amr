...
# -*- coding: utf-8 -*-
"""
Created on Mon Feb 10 11:38:42 2020
...
#%%
new_table= cursor.execute("CREATE TABLE IF NOT EXISTS Candida_auris_nov_ast AS SELECT * FROM Candida_auris WHERE AST_phenotypes LIKE '%=S%' OR AST_phenotypes LIKE '%=R%' OR AST_phenotypes LIKE '%=I%' ")
...
#%%
new_table= cursor.execute("CREATE TABLE IF NOT EXISTS Shigella_nov_ast_us AS SELECT * FROM ShigellaMerged  WHERE geo_loc_name LIKE '%USA%'  ")
print("Selected %s rows" %new_table)
print("Selected %s rows " %cursor.rowcount)
#rows =cursor.fetchall()#fetch all rows at once
#print(tabulate(rows, headers=['strain','AST_phenotype', 'AMR_genotype'], tablefmt='psql'))
...
#%%
import pandas as pd
numrows= cursor.execute("SELECT AST_phenotypes FROM Shigella_nov_ast_us")
